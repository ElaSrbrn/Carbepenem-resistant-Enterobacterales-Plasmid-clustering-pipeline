...
For plotting of the pling output (all_plasmid_distances.tsv) and based on the mash distance filtering metrics:
# Final version: circular layout, dark colors, bold 0-edges, evenly spaced external labels
...
# === Load data ===
csv_path = "DCJ_network_table.csv"  # <-- Update path if needed
df = pd.read_csv(csv_path)

# === Build graph from contingency table ===
G = nx.Graph()

def pretty(name: str) -> str:
    """Create cleaner display labels for nodes."""
    name = str(name).strip()
...
row_nodes = df["Sample Room"].tolist()
col_nodes = [c for c in df.columns if c != "Sample Room"]
all_nodes = set(row_nodes) | set(col_nodes)
for n in all_nodes:
    G.add_node(n, label=pretty(n))

for _, row in df.iterrows():
    u = row["Sample Room"]
    for v in col_nodes:
        val = row[v]
        if pd.isna(val) or str(val).lower() == "x":
            continue
        w = int(val)
        if not G.has_edge(u, v):
            G.add_edge(u, v, weight=w)

# === Remove nodes (per user instruction) ===
G.remove_nodes_from(["3 C", "Drain B"])

# === Color scheme (dark tones) ===
color_map = {
    "9 D": "#003366",      # dark blue
    "10 D": "#003366",     # dark blue
    "4 A,D": "#003366",    # dark blue
    "2 B,D": "#003366",    # dark blue
    "8 B": "#8B0000",      # dark red
    "Drain A": "#8B0000",  # dark red
    "1 A": "#8B0000",      # dark red
    "Drain C": "#006400",  # dark green
    "7 C": "#006400",      # dark green
    "5 D": "#5D3FD3",      # dark purple
}

# === Layout setup ===
def sort_key(n):
    if str(n).startswith("Drain"):
        return (2, n)
    try:
...
ordered_nodes = sorted(G.nodes(), key=sort_key)

# Circular layout with slight expansion for labels
pos = nx.circular_layout(ordered_nodes, scale=2.2)

# Compute outward label positions and alignment
label_radius_factor = 1.35
label_positions = {}
label_align = {}
for node, (x, y) in pos.items():
    ...
    if r == 0:
        r = 1e-6
    ux, uy = x / r, y / r
    lx, ly = ux * (r * label_radius_factor), uy * (r * label_radius_factor)
    label_positions[node] = (lx, ly)
    label_align[node] = 'left' if lx >= 0 else 'right'

# === Draw ===
plt.figure(figsize=(12, 12))
node_colors = [color_map.get(n, "grey") for n in ordered_nodes]

# Nodes
nx.draw_networkx_nodes(G, pos, nodelist=ordered_nodes, node_color=node_colors, node_size=800)

# Edges (bold where weight == 0)
edges_zero = [(u, v) for u, v, d in G.edges(data=True) if d["weight"] == 0]
edges_nonzero = [(u, v) for u, v, d in G.edges(data=True) if d["weight"] != 0]

nx.draw_networkx_edges(G, pos, edgelist=edges_nonzero, width=1.2, edge_color="black")
nx.draw_networkx_edges(G, pos, edgelist=edges_zero, width=4, edge_color="black")

# Node labels (outside circle)
for n in ordered_nodes:
    x, y = label_positions[n]
    plt.text(
        x, y, G.nodes[n]["label"],
        fontsize=12, fontweight="bold",
        ha=label_align[n], va='center'
    )

# Edge labels (DCJ distances)
